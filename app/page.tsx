...
async function FeaturedProductsWrapper() {
  const { getFeaturedProducts } = await import('@/lib/db/queries');
  const featured = await getFeaturedProducts(8);
  return <FeaturedProducts featured={featured} />;
}

async function CategoriesWrapper() {
  const { getCategories } = await import('@/lib/db/queries');
...
function CategoriesSkeleton() {
  return (
    <section className="border-border w-full border-t">
      <div className="mx-auto w-full max-w-7xl px-4 py-16">
