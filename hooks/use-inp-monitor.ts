        ...
        if (metric.value > 200) {
          console.group(
            ...
            metric.rating === 'poor'
              ...
              : 'color: #BA7517; font-weight: bold'
          )
          console.log('Interaction:', attr.interactionType, 'on', attr.interactionTarget)
          console.log('Input delay:', Math.round(
            attr.processedEventEntries[0]?.processingStart -
            attr.processedEventEntries[0]?.startTime
          ) + 'ms')
          console.log('Processing:', Math.round(
            attr.processedEventEntries[0]?.processingEnd -
            attr.processedEventEntries[0]?.processingStart
          ) + 'ms')
          console.log('Presentation:', Math.round(
            attr.nextPaintTime -
            attr.processedEventEntries[0]?.processingEnd
          ) + 'ms')
